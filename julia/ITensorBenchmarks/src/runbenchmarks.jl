                ...
                for maxdim in maxdims_benchmark,
                        ...
                        if isnothing(cpp_or_julia) || cpp_or_julia == "julia"
                                ...
                                if write_results
                                        ...
                                        filepath = joinpath(filepath, "run.txt")
...
function runbenchmarks(; blas_num_threads = OPTIONS["blas_num_threads"],
                       ...
                       benchmarks = OPTIONS["benchmarks"],
                       splitblocks = OPTIONS["splitblocks"],
                       # Remove the logfile parameter and its default value
                       kwargs...)
    # Define a simple, fixed logfile path directly
    # This will create a file named "run.txt" in your current working directory
    # where you execute the script.
    # If you want it in a specific 'logs' subdirectory, you can add `joinpath(pwd(), "logs", "run.txt")`
    
    # Option 1: Simple run.txt in the current directory
    logfile_path = "run.txt"

    # Option 2: Simple run.txt in a 'logs' subdirectory (recommended for organization)
    # Be sure to create the 'logs' directory if it doesn't exist.
    # logfile_dir = joinpath(pkgdir(@__MODULE__), "logs")
    # mkpath(logfile_dir) # Ensure the directory exists
    # logfile_path = joinpath(logfile_dir, "run.txt")

    println(stdout, "Saving information about the benchmark run to the file $logfile_path")
    println(stdout)
...
# You can now completely remove or comment out the `logfile_name` function:
