...
int main() {
  // 1.a) Like in ITensors, define some indices, say 'a', 'b', 'c' with some
  // dimensions.
  ...
  // 1.c) Define a vector that actually holds the dimensions of the indices of
  // A, mapping values from extent!
  ...
  size_t sizeA = sizeof(floatTypeA) *
                 elementsA; // Think of storing the tensor as a very long array

  // 2.a) Allocate memory for a pointer A_d on the device
  void *A_d;
  cudaMalloc((void **)&A_d, sizeA);

  // 2.b) Allocate memory for a pointer A on host and fill up the values of the
  // tensor
  ...
  HANDLE_ERROR(cutensorCreateTensorDescriptor(
      handle, &descA, nmodeA, extentA.data(), NULL, typeA, kAlignment));
  ...
  HANDLE_ERROR(cutensorCreateContraction(
      ...
      modeB.data(), CUTENSOR_OP_IDENTITY, descCompute));
